공용 유틸리티를 아래처럼 제안드립니다. 이름은 `Guard`로 하였고, 요청하신 방식대로 **메시지 전달 시 `Supplier<String>`로 지연 평가**합니다. 또한 상황에 맞게 `NullPointerException`, `IllegalArgumentException`, `IllegalStateException`, `UnsupportedOperationException`을 던지도록 분리했습니다.

```java
package com.yourcompany.shared.util;
...
```java
var userId = Guard.notNull(inputUserId, () -> "userId must not be null");
var name   = Guard.hasText(command.name(), () -> "name required");
Guard.isTrue(quantity > 0, () -> "quantity must be positive");
Guard.state(orderStatus.isCancellable(), () -> "order cannot be cancelled now");
```

설명 요약

* **메시지 제공 시 Supplier 필수**: 모든 가변 메시지는 실제로 예외가 발생하는 경우에만 계산됩니다.
* **예외 타입 분리**: 입력값 검증은 `IllegalArgumentException`, 상태 위반은 `IllegalStateException`, null은 `NullPointerException`, 미지원 경로는 `UnsupportedOperationException`.
* **체이닝/한 줄 할당**: `notNull`, `notBlank/hasText`는 입력값을 그대로 반환합니다.